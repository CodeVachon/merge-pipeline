---
id: 53
title: "Docs: README version management section, CHANGELOG, install.sh wording"
state: Done
parent: 50
assignee: claude-code
labels: [self-update, cli]
blockedBy: [52]
created: 2026-09-23T15:16:17Z
updated: 2026-09-23T15:29:59Z
---

## Description

README "Keeping it up to date" gains a table of `upgrade`, `versions`, `use`, `versions remove`, `versions prune` and a sentence that switching takes effect on the next command because PATH points at the stable `current` link. install.sh's closing advice keeps the PATH note (that one is genuinely first-install only) but must not imply a restart is needed after upgrades. CHANGELOG Unreleased entries. Motte note recording that no shell restart was ever required and why.

## Notes

### 2026-09-23T15:29:59Z — claude-code (agent)

README "Keeping it up to date" now has a "Managing installed versions" table (versions / use / versions remove / versions prune) and a paragraph stating that no shell restart is needed and why (PATH -> ~/.local/bin/merge-pipeline -> current/bin -> version dir; only `current` moves). install.sh's closing lines no longer say "open a new terminal"; the PATH advice above them stays because it is genuinely first-install only. CHANGELOG Unreleased: Added (three commands) and Changed (upgrade's closing line). layout::install_sh_agrees_with_this_layout still passes.
